...
async function fetchProduct(id: string): Promise<ProductProps> {
  const product = await fetch(`http://localhost:5001/products/${id}`, {
    cache: "no-store",
  });
...
async function fetchRelatedProduct(): Promise<ProductProps[]> {
...
export default async function ShopDetai({
  ...
  const relatedProduct = await fetchRelatedProduct();
  ...
  return (
    <>
      ...
      <RelatedProducts productItems={relatedProduct} />
    </>
  );
}
